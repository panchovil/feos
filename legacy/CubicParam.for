...
!        if(NMODEL<=2)READ(NIN,*)comp, Tc,Pc,OM      ! Activate for a list (SRK, PR)
!        if(NMODEL==3)READ(NIN,*)comp, Tc,Pc,OM,del1   ! Activate for a list (RKPR)
	IF(NMODEL.EQ.4)GO TO 104
	IF(nmodel.LE.2)THEN
		IF(ICALC.EQ.0)THEN		! CONSTANTS SPECIFICATION
			READ(NIN,*)Tc,Pc,OM            ! Activate when it is not a list
			RT=RGAS*Tc
...
c		write(nout,*)'Parameters were specified and constants calculated'
	ELSE  ! CONSTANTS SPECIFICATION (ICALC = 0,2,3)
		...
		ELSE
			...
			ELSE
				if (OM<=0.491) rm=0.37464 +1.54226*OM-0.26992 *OM**2  ! m from PR
			    if (OM >0.491) rm=0.379642+1.48503*OM-0.164423*OM**2+0.016666*OM**3 ! PR78
			END IF
		END IF
