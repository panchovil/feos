...
contains
   ...
   ! --------------------------------------------------------------------------
   subroutine classic_mix(self, compounds, d_mixture, b_mixture, d1_mixture)
       ...
       type(scalar_property), allocatable :: a(:), b(:), d1(:)
       ...
       allocate(a(n), b(n), d1(n), moles(n))

       do i=1,n
           a(i)     = compounds(i)%a_parameter()
           b(i)     = compounds(i)%b_parameter()
           d1(i)    = compounds(i)%del1
           moles(i) = compounds(i)%moles
       end do

       aij = aij_classic(n, a, self%kij)
       bij = aij_classic(n, b, self%lij)

       d_mixture  = d_mix(n, aij, moles)
       b_mixture  = b_mix(n, bij, moles)
       d1_mixture%val = compounds(1)%del1     ! All derivatives 0 for a two parameters eos
   ...
   function aij_classic(n, a, kij) result(aij)
      ...
      do i=1,n
         do j=1,n
         aij%val(i, j) = sqrt(a(i)%val * a(j)%val) * (1 - kij(i, j))
         end do
      end do
   ...
   pure function bij_classic(n, b, lij) result(bij)
      ...
      do j=1, n
         bij%val(j, j) = b(j)%val
         bij%val(:, j) = (b(:)%val + b(j)%val)/2 * (1 - lij(:, j))
      end do
   end function bij_classic

   function d_mix(n, aij, moles) result(d)
   ...
   function b_mix(n, bij, moles) result (b)
   ...
   function delta1_mix(n, delta1, moles) result(d1)
       integer, intent(in) :: n
